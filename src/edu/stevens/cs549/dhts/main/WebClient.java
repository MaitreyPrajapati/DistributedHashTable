...
public class WebClient {
	...
	private GenericType<JAXBElement<TableRow>> RowType = new GenericType<JAXBElement<TableRow>>() {};
	...
	//Client call to get successor node
			...
			public NodeInfo closestPrecedingFinger(NodeInfo node, int id) throws DHTBase.Failed{
				UriBuilder cpfPath = UriBuilder.fromUri(node.addr).path("closestPrecedingFinger");
				URI getPath = cpfPath.queryParam("id", id).build();
			...
			public String[] get(NodeInfo n, String k) throws DHTBase.Failed {
				UriBuilder path = UriBuilder.fromUri(n.addr).path("getValue");
				URI getPath = path.queryParam("key", k).build();
				info("client getValue(" + getPath + ")");
				Response response = getRequest(getPath);
				if (response == null || response.getStatus() >= 300) {
					throw new DHTBase.Failed("GET /getValue?id=ID");
				} else {
					return response.readEntity(RowType).getValue().vals;
				}
			}
		
			//addMethod
			public void add(NodeInfo Info, String key, String value) throws DHTBase.Failed {
				// TODO Auto-generated method stub
				UriBuilder path = UriBuilder.fromUri(Info.addr).path("add");
				URI addPath = path.queryParam("key", key).queryParam("value", value).build();
			...
			//deleteMethod
			public void delete(NodeInfo Info, String key, String value) throws DHTBase.Failed {
				UriBuilder path = UriBuilder.fromUri(Info.addr).path("delete");
				URI deletePath = path.queryParam("key", key).queryParam("value", value).build();
